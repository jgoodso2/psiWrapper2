...
{
    public class PwaUpdateProjectPlanCommand : IPwaCommand, IPwaCommandFactory, IPwaOutput
    {
        ...
        public string PwaCommandDescription
        {
            get
            {
                return "PwaUpdateProjectPlanCommand - updates a resource plan";
            }
        }
        ...
        public string PwaCommandName
        {
            get
            {
                return "PwaUpdateProjectPlanCommand";
            }
        }
        ...
        public void Execute()
        {
            try {
                ...
                OutputResult = UpdateProjectPlanAsync(PwaInput.ResourcePlans, controller, PwaInput.Timescale, PwaInput.Workscale,
                    PwaInput.StartDate, PwaInput.EndDate);
            }
            catch (Exception ex)
            {
        ...
        {
            return new PwaUpdateProjectPlanCommand() { _pj = pj, PwaInput = (PwaProjectPlanInput)new PwaProjectPlanInput(pwaInput).ParseInput() };
        }
        ...
        private UpdateResult[] UpdateProjectPlanAsync(string json, ResourcePlanController controller, string timeScale,
            string workScale, string startDate, string endDate)
        {
            try
            {
               //get json data
                ProjectPlan[] rp = (ProjectPlan[])Newtonsoft.Json.JsonConvert.DeserializeObject(json, (typeof(ProjectPlan[])));
                //pivot data into project based resplans
                UpdateResPlan[] resPlans = ProjectPlan.GetUpdateResPlans(rp);
                //create output data
                UpdateResult[] results = new UpdateResult[resPlans.Count()];
